...
const subpointExamSummaries = computed<Record<string, SubpointExamSummary>>(() => {
  ...
  for (const subpoint of props.article.subpoints) {
    const subpointBlockIds = new Set(subpoint.blocks.map((block) => block.id))
...
function renderInlineMath(text: string): string {
  ...
  return text
    .replace(
      ...
      (_, f: string) => {
        try {
          return katex.renderToString(f.trim(), { displayMode: true, throwOnError: false, trust: false })
        } catch { return `$$${f}$$` }
      },
    )
    .replace(
      ...
      (_, f: string) => {
        try {
          return katex.renderToString(f.trim(), { displayMode: false, throwOnError: false, trust: false })
        } catch { return `$${f}$` }
      },
    )
}
  ...
  <article class="space-y-4">
    ...
    >
      <header class="mb-6 flex flex-wrap items-baseline gap-x-7 gap-y-3">
        <h2 class="m-0 text-[clamp(1.55rem,2vw,1.9rem)] font-semibold leading-tight tracking-[-.035em] text-[#071225]">
          <span class="mr-3 font-mono text-[.62em] font-bold text-[#31559e]">{{ String(index + 1).padStart(2, '0') }}</span>{{ subpoint.title }}
        </h2>
        ...
        <RouterLink
          ...
          :to="{
            name: 'exams',
            query: { knowledgeBlockIds: subpointExamSummaries[subpoint.id].blockIds.join(',') },
          }"
        ...
        >
          <span class="font-bold text-[#12327f]">{{ subpointExamSummaries[subpoint.id].examCount }}</span>
          <span>道关联真题</span>
      ...
      <div class="grid gap-7">
        <section
          ...
          class="scroll-mt-28"
        >
          <KnowledgeMarkdown v-if="block.type === 'paragraph'" :source="block.text" />
          ...
          <figure v-else-if="block.type === 'image'" class="m-0 border-y border-[#e1e6ed] bg-[#fafbfd] px-4 py-7">
            <img :src="block.src" :alt="block.alt" class="mx-auto block h-auto max-h-[60vh] max-w-full w-auto" loading="lazy" />
            <figcaption v-if="block.caption" class="mt-4 text-center text-[13px] leading-6 text-slate-500">图 · {{ block.caption }}</figcaption>
          </figure>

          <figure v-else-if="block.type === 'formula'" class="m-0 overflow-x-auto border-y border-[#e1e6ed] bg-[#f7f9fc] px-6 py-6 text-[19px]">
            <div v-html="renderFormula(block.formula)"></div>
            <figcaption v-if="block.caption" class="mt-1 text-sm leading-7 text-slate-500">{{ block.caption }}</figcaption>
          </figure>
          ...
          >
            <div class="flex items-center gap-2.5">
              <svg class="h-[18px] w-[18px] shrink-0" viewBox="0 0 20 20" aria-hidden="true">
                <circle cx="10" cy="10" r="10" fill="#f59e0b" />
                <path d="M10 5.5v5.5" stroke="#fff" stroke-width="1.9" stroke-linecap="round" />
                <circle cx="10" cy="14.2" r="1.35" fill="#fff" />
              </svg>
              <strong class="text-[15px] font-bold leading-7 text-[#071225]">{{ block.title }}</strong>
            </div>
            <p class="mb-0 mt-1 text-[17px] leading-8 text-slate-600" v-html="renderInlineMath(block.text)"></p>
          </aside>
