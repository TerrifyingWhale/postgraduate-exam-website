...
function loadOpenChapterId() {
  ...
  try {
    ...
    return Array.isArray(saved.openChapters) &&
      typeof saved.openChapters[0] === "string"
...
function openChapter(chapterId: string) {
  ...
  try {
    window.localStorage.setItem(
      SIDEBAR_STATE_KEY,
      JSON.stringify({ openChapterId: chapterId }),
    );
  } catch {
...
function toggleChapter(chapterId: string) {
  ...
  try {
    window.localStorage.setItem(
      SIDEBAR_STATE_KEY,
      JSON.stringify({ openChapterId: openChapterId.value }),
    );
  } catch {
    // Storage may be unavailable in private or embedded browsing contexts.
  }
...
<template>
  ...
  >
    <div
      class="mb-2 px-3 py-2 font-mono text-[10px] font-bold tracking-[.15em] text-slate-400"
    >
      CONTENTS / 目录
    </div>
    <ol class="m-0 list-none space-y-1 p-0">
      <li
        ...
        :class="
          openChapterId === chapter.id ? 'bg-white ring-1 ring-[#e3e9f1]' : ''
        "
      >
        <button
          type="button"
          ...
          <ul class="chapter-sections-inner m-0 list-none px-1.5 pb-2 pt-1">
            <li v-for="section in chapter.sections" :key="section.id">
              <button
                ...
                "
                :aria-current="
                  section.id === activeSectionId ? 'page' : undefined
                "
    ...
    <p v-if="!visibleChapters.length" class="px-3 py-8 text-sm text-slate-400">
      没有匹配的章节或知识点。
    </p>
  </nav>
</template>
